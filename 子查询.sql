1.子查询是指在另一个查询语句中的SELECT子句。
  ...
  其中， SELECT * FROM t1 ...称为 Outer Query[外查询](或者Outer Statement),    SELECT column1 FROM t2 称为  Sub Query[子查询]。

  所以，我们说子查询是嵌套在外查询内部。而事实上它有可能在子查询内部再嵌套子查询。 子查询必须出现在圆括号之间。 行级子查询 返回结果最多为一行。
...
-- 查询价格大于或等于"超极本"价格的商品，并且按价格降序排列
   ...
   SELECT goods_id,goods_name,goods_price FROM tdb_goods 
   WHERE goods_price = ANY(SELECT goods_price FROM tdb_goods WHERE goods_cate = '超极本')
   ORDER BY goods_price DESC;
...
--  = ANY 或 = SOME 等价于 IN
   ...
   SELECT goods_id,goods_name,goods_price FROM tdb_goods 
   WHERE goods_price IN (SELECT goods_price FROM tdb_goods WHERE goods_cate = '超极本')
   ORDER BY goods_price DESC; 
...
-- 将刚才查询到的子类数量更新到tdb_goods_types数据表
  ...
  UPDATE tdb_goods_types AS t1 
    INNER JOIN 
      ...
      ON t1.type_id = t2.type_id SET t1.child_count = t2.children_count ;
...
-- 删除重复记录
  ...
  FROM tdb_goods AS t1 
    LEFT JOIN 
      ...
      ON t2.goods_name = t1.goods_name 
